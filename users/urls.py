...
urlpatterns = [
    path("register/", register, name="register"),  # Маршрут для регистрации нового пользователя
    ...
    path("profile/edit/", edit_profile, name="edit_profile"),  # Маршрут для редактирования профиля пользователя
    path("logout/", LogoutView.as_view(), name="logout"),  # Маршрут для выхода из аккаунта
]
