    ...
    def validate_forbidden_words(self, text, field_name):
        ...
        for word in self.forbidden_words:
            if word in text.lower():
                raise forms.ValidationError(f"{field_name.capitalize()} содержит запрещенное слово: {word}")
...
class VersionForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # Проверяем, если версия помечена как текущая, убедиться, что у продукта нет другой текущей версии
        if is_current and Version.objects.filter(product=product, is_current_version=True).exclude(pk=self.instance.pk).exists():
